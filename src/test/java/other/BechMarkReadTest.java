...
public class BechMarkReadTest {
    ...
    static class Reader implements Runnable {
        ...
        public Reader(boolean useScore, int start, int end) {
            this.start = start;
            this.end = end;
...
//                req.preference("_only_nodes:kZt7TSMOSZiuUKSRsj8fBA,kmhm3RNwS_aPJmZnQySkQw");
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        boolean useScore = Boolean.parseBoolean(args[1]);
        log.info("parrll={}", parrll);


        for (int i = 0; i < parrll; i++) {
            new Thread(new Reader(useScore, 0, 2000)).start();
        }
